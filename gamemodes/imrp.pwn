...
#define         SERVER_VERSION				"IM:RP v3.11b"
#define         SERVER_MAP					"Las Venturas"
#define         SERVER_SITE					"www.imperiousgaming.net"
...
forward MySQL_Offline_Ban(name[], reason[], days, adminid, MySQL:connectionHandle);
forward SetServerToRestart();
forward BackupTimer(playerid);
forward MySQL_Delete_Number(number, playerid, MySQL:connectionHandle);
forward MySQL_Player_Referral(playerid, referer[], MySQL:connectionHandle);
...
#define LOCATION_TAXIJOB 			2204.1160,1394.0668,11.0625
...
#define LOCATION_MINERJOB   	    599.9070,872.1904,-42.9609
...
{
	AntiDeAMX();
	SendRconCommand("hostname [OPEN BETA] "SERVER_NAME);
...
*/
	...
	Login2 = TextDrawCreate(235.666702, 27.377784, "Imperious Roleplay");
	...
	Login5 = TextDrawCreate(279.000091, 9.125836, "WWW.imperiousgaming.net");
	...
	Login8 = TextDrawCreate(242.333312, 393.244415, "TS3: ts.imperiousgaming.net");
...
public OnPlayerConnect(playerid)
{
	...
	mysql_format(g_SQL, PlayerInfo[playerid][pUsername], MAX_PLAYER_NAME, "%e", PlayerInfo[playerid][pUsername]);
...
public OnQueryFinish(query[], resultid, extraid, MySQL:connectionHandle)
{
	...
	switch (resultid)
	{
	    ...
	    case THREAD_ACCOUNT_REGISTER:
	    {
			...
			format(szQuery, sizeof(szQuery), "SELECT * FROM `accounts` WHERE `Username` = '%s' LIMIT 0, 1", PlayerInfo[extraid][pUsername]);
	        mysql_tqueryof(szQuery, THREAD_REGISTER_SPAWN, extraid, g_SQL);
	    }
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	switch(dialogid)
	{
	    case DIALOG_REGISTER:
	    {
			...
			mysql_tqueryof(szQuery, THREAD_ACCOUNT_REGISTER, playerid, g_SQL);
		...
		case DIALOG_CHANGE_PASSWORD:
		{
			...
			mysql_tqueryof(string, THREAD_NO_RESULT, playerid, g_SQL);
		...
		case DIALOG_WAYPOINT_SUBMIT:
		{
		    ...
		    switch (WaypointEditOption[playerid])
		    {
		        ...
		        case 19:
		        {
					...
					mysql_tqueryof(string, THREAD_LOG_RESULT, playerid, g_SQL);
		        }
		...
		case DIALOG_BUSINESS_EDIT:
		{
		    ...
		    switch (listitem)
		    {
		        ...
		        case 2:
		        {
					...
					mysql_tqueryof(string, THREAD_LOG_RESULT, INVALID_PLAYER_ID, g_SQL);
		        }
		    }
		}
		...
		case DIALOG_FACTION_ADMIN:
		{
		    ...
		    switch (listitem)
		    {
		        ...
		        case 6:
		        {
					...
					mysql_tqueryof(szQuery, THREAD_LOG_RESULT, playerid, g_SQL);
...
public MySQL_Change_Name(playerid, newname[], adminid, MySQL:connectionHandle)
{
	...
	if(rowCount == 0)
	{
		...
		mysql_tqueryof(string, THREAD_LOG_RESULT, playerid, connectionHandle);
...
{
	SendClientMessageEx(playerid, COLOR_GREEN, "Server Information{FFFFFF} | {33AA33}SA-MP:{FFFFFF} samp.imperiousgaming.net | {33AA33}Website:{FFFFFF} imperiousgaming.net | {33AA33}TS3:{FFFFFF} ts.imperiousgaming.net");
	return 1;
}
...
public MySQL_Unban_Player(name[], adminid, MySQL:connectionHandle)
{
...
public MySQL_Suspend_Player(name[], adminid, MySQL:connectionHandle)
{
	new szQuery[128], rowCount;
	cache_get_row_count(rowCount);
	if(rowCount > 0)
	{
		...
		mysql_tqueryof(szQuery, THREAD_LOG_RESULT, INVALID_PLAYER_ID, g_SQL);
...
public MySQL_Offline_Ban(name[], reason[], days, adminid, MySQL:connectionHandle)
{
	new
	    szString[256],
	    szIpAddress[17],
	...
	if(rowCount > 0)
	{
		...
		mysql_tqueryof(szString, THREAD_NO_RESULT, adminid, connectionHandle);
		if(days == -1)
		{
		...
		mysql_tqueryof(szString, THREAD_LOG_RESULT, INVALID_PLAYER_ID, g_SQL);
...
CMD:query(playerid, params[])
{
	if(IsPlayerAdmin(playerid))
    {
		...
		mysql_tqueryof(params, THREAD_LOG_RESULT, playerid, g_SQL);
...
public MySQL_Player_Referral(playerid, referer[], MySQL:connectionHandle)
{
...
public MySQL_Delete_Number(number, playerid, MySQL:connectionHandle)
{
	new
	    szQuery[128],
		rowCount;
	...
	if(rowCount > 0)
	{
		...
		mysql_tqueryof(szQuery, THREAD_NO_RESULT, playerid, connectionHandle);
...
CMD:accept(playerid, params[])
{
	...
	else if(!strcmp(option, "ticket", true))
	{
		...
		if(GetPVarInt(ticketOfficer, "TicketPlayer") == playerid)
		{
	    	...
	    	AddVaultMoney(1, ticketAmount); // add to government vault
...
public OnPlayerDisconnect(playerid, reason)
{
	if(GetPVarInt(playerid, "gPlayerLogged") == 1)
	{
		...
		PlayerTextDrawDestroy(playerid, JailTimerTD[playerid]);
	 	...
	 	SavePlayerAccount(playerid, g_SQL);
	}
	return 1;
}
...
public OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid, bodypart)
{
	...
	if(IsPlayerRestricted(playerid) || GetPVarType(playerid, "AdminDuty")) return 0;
    RemovePlayerColorFade(playerid);
...
stock UnbanPlayerEx(name[])
{
	...
	mysql_tqueryof(szQuery, THREAD_NO_RESULT, INVALID_PLAYER_ID, g_SQL);
...
stock MySQL_Update_Gate(id)
{
	...
	mysql_tqueryof(szQuery, 0, INVALID_PLAYER_ID, g_SQL);
}
...
public LoadPlayerAccount(playerid, MySQL:connectionHandle)
{
	if(!IsPlayerConnectedEx(playerid)) return 0;
...
stock MySQL_Save_Player_Vehicle(playerid, i)
{
	...
	mysql_tqueryof(szString, THREAD_NO_RESULT, playerid, g_SQL);
}

public SavePlayerAccount(playerid, MySQL:connectionHandle)
{
	if(GetPVarInt(playerid, "gPlayerLogged") == 0) return 0;
...
stock AddBusinessEarnings(b, amount)
{
	...
	format(string, sizeof(string), "UPDATE `businesses` SET `Earnings` = '%d' WHERE `Id` = '%d'", BusinessInfo[b][bEarnings], b);
	mysql_tqueryof(string, THREAD_NO_RESULT, INVALID_PLAYER_ID, g_SQL);
...
stock SQLLog(name[], type[], iname[], reason[])
{
	...
	return mysql_tqueryof(szQuery, THREAD_NO_RESULT, INVALID_PLAYER_ID, g_SQL);
}
